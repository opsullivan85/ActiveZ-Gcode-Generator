...
class pSettings(object):
    ...
    settings['ePerDst'] =       'Extrusion per distance__________'
...
class gcode:
    ...
    def Polar2GCode(self, objH, objR, Θres, fx, progress=True, **kwargs):
        ...
        for l, layer in enumerate(layers):  # @UnusedVariable
            ...
            if(layer == 0):
                for a, angle in enumerate(Θ):  # @UnusedVariable
                    self.G1((x[a],y[a],z[a][l]), extrusion=ePerMove, ePercent=self.pSettings.fEMod)
            
            #For Other Layers
            else:
                for a, angle in enumerate(Θ):  # @UnusedVariable
                    if(a==0): #if first changing layer
    ...
    def G1(self, coords, extrusion, setFeed=None, ePercent=1):
        ...
        Keyword Args:
            ...
            setFeed (float): Feed to change to 
            ePerDst (float): Scalar applied to the extrusion ammount
            ePercent (float): Extra ammount as percent to extrude
        """
