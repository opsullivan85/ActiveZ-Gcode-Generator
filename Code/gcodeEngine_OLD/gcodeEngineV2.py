...
class gcode:
    ...
    #    
    def __init__(self, fname="gcode.GCODE", openfile=True, fileMode="a", initfname=False, fillamentDia=1.75, nozzleWidth=0.4, precision = 5):  # @UnusedVariable
        self.precision = precision
        self.fillamentDia = 1.75
        self.nozzleWidth = 0.4
    ...
    def Polar2GCode(self, objH, objR, Θres, fx, 
                    layerH=0.2, printOffsets=(150,150), progress=True, 
                pFeed=1200, fLayerH=None, fLayerEMod=1.2, eMod=1, **kwargs):
        ...
        Keyword Args:
            ...
            layerH (float): The height of each normal layer
            printOffsets (float[2]): The offset from (0,0) to print the object
            progress (bool): Whether or not show generation progress
            pFeed (int): Extruder feed to use for normal printing
            fLayerH (bool): Height of the first layer
            fLayerEMod (float): Extrusion modifier to use for the first layer
            eMod (float): Global extrusion modifier
        """
        
        fLayerH = layerH if fLayerH == None else fLayerH
        Θ = np.linspace(0, 2*π, Θres)
        r = objR
        layers = np.arange(0, objH-fLayerH, layerH)
        numLayers = layers.size
        
        #2*π*r = circumference
        #self.nozzleWidth*layerH = crossectional area
        #2*π*r*self.nozzleWidth*layerH* volume of layer
        ePerMove = 2*π*r*self.nozzleWidth*layerH*self.eConst/Θ.size
        
        x = r*np.cos(Θ) + printOffsets[0]
        #x[Θ]
        y = r*np.sin(Θ) + printOffsets[1]
        #y[Θ]
        z = fx(layers,Θ,**kwargs)+fLayerH
        #z[Θ][layer]
        ...
        self.f.write("\n;Setting Feed\n")
        self.coords[3] = pFeed
        self.f.write(f"G1 F{pFeed:.{self.precision}f}\n")
        ...
        for l, layer in enumerate(layers):  # @UnusedVariable
            ...
            for a, angle in enumerate(Θ):  # @UnusedVariable
                if(a>0 or layer == 0):
                    if(layer == 0):
                        self.G1((x[a],y[a],z[a][l]), e=ePerMove, ePercent=fLayerEMod)
                    else:
                        self.G1((x[a],y[a],z[a][l]), e=ePerMove, ePercent=eMod)
                else:
    ...
    def G1(self, coords, f=False, eScalar = 0.0333, ePercent=1, e=False):
        """
        Description:
    ...
    def travelMove(self, coords, tRetr = 0.0914, tComp = 0.0457, tZGap = 3, tFeed = 5000, **kwargs):   # @UnusedVariable
        """
        Description:
        ...
        Keyword Args:
            coords (float[3]): Coords to move to
            tRetr (float): Ammount to retract
            tComp (float): Ammount to compensate
            tZGap (float): Ammount to raise Z
            tFeed (float): Feed to move at
        """
        
        #Set feed
        self.f.write(f"\n;Travel Move\n")
        self.f.write(f"G0 F{tFeed:.{self.precision}f}\n")
        #Raise z and retract
        self.f.write(f"G1 Z{(self.coords[2]+tZGap):.{self.precision}f} E{(self.coords[4]-tRetr):.{self.precision}f}\n")
        #Goto coords
        self.f.write(f"G0 X{coords[0]:.{self.precision}f} Y{coords[1]:.{self.precision}f} Z{(coords[2]+tZGap):.{self.precision}f}\n")
        #Lower z
        self.f.write(f"G0 Z{coords[2]:.{self.precision}f}\n")
        #Compensate and unretract
        self.f.write(f"G1 E{(self.coords[4]+tComp):.{self.precision}f}\n")
        #Reset feed
    ...
    def finish(self, zGap = 3, retr = 0.0914, endOffsets = (20,20), tFeed = 5000, **kwargs):  # @UnusedVariable
        """
        Description:
        ...
        self.f.write(f"G0 F{tFeed:.{self.precision}f}\n")
        #Raise z and retract
        self.f.write(f"G1 Z{(self.coords[2]+zGap):.{self.precision}f} E{(self.coords[4]-retr):.{self.precision}f}\n")
        #Goto coords
        self.f.write(f"G0 X{endOffsets[0]:.{self.precision}f} Y{endOffsets[1]:.{self.precision}f}\n")
        #Update Coords
        self.coords[0] = endOffsets[0]
        self.coords[1] = endOffsets[1]
        self.coords[2] += zGap
        self.coords[4] -= retr
    #
...
def main(output = False):
    tmp = gcode("wigglyTube (11) 00.GCODE", fileMode="w")
    tmp.appendFile("startGcode.txt")
    tmp.Polar2GCode(objH=25, objR=10, Θres=100, layerH=0.2, pFeed=500, tFeed=5000, tERetract=0, tEComp=0.0457, progress=output,
                    fx=fx, amp=0, freq=8, zPeriod=25)
    ...
    tmp = gcode("wigglyTube (11) 04.GCODE", fileMode="w")
    tmp.appendFile("startGcode.txt")
    tmp.Polar2GCode(objH=25, objR=10, Θres=100, layerH=0.2, pFeed=500, tFeed=5000, tERetract=0, tEComp=0.0457, progress=output,
                    fx=fx, amp=0.4, freq=8, zPeriod=25)
    tmp.appendFile("endGcode.txt")
